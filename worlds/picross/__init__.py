...
class PicrossWeb(WebWorld):
    tutorials = [
        Tutorial(
            "Multiworld Setup Guide",
            "A guide to setting up Picross. This guide covers single-player, multiworld, and website.",
            "English",
            "setup_en.md",
            "setup/en",
            ["Spineraks"],
...
@dataclass
class PicrossOptions(PerGameCommonOptions):
    size_of_grid: SizeOfGrid

class PicrossWorld(World):
    game: str = "Picross"
    options_dataclass = PicrossOptions
    web = PicrossWeb()
    item_name_to_id = {"Picross clues": 67}
    ...
    def generate_early(self):
        ...
        self.multiworld.itempool += [self.create_item("Picross clues", 67) for _ in range(num_steps)]
        self.multiworld.push_precollected(self.create_item("Picross clues", 67))
        ...
        for i, loc in enumerate(menu.locations):
            set_rule(loc, lambda state, step=i+1: state.has(f"Picross clues", self.player, step))
        
        #victory - have number of clues equal to number of steps
        self.multiworld.completion_condition[self.player] = lambda state: state.has("Picross clues", self.player, num_steps)
